...
import os
import numpy as np
# os.environ in Python is a mapping object that represents the user’s environmental variables and returns dictioary
...
class SentiProcessor(DataProcessor):
    """Processor for Senti dataset."""  # splitting whole dataset in three subsets: training,developement and testing

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            open(os.path.join(data_dir, " ** path of input file(text format) ** ")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            open(os.path.join(data_dir, "  **path of input file(text format)** ")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            open(os.path.join(data_dir, "  ** path of input file(text format) ** ")), "test")

    def get_labels(self):  # performs labeling
        """See base class."""
    ...
    def find_with_pattern(self, pattern, s, replace=False, tag=None):
        if replace and tag == None:
            raise Exception(
                "Parameter error", "If replace=True you should add the tag by which the pattern will be replaced")
        regex = re.compile(pattern)
        if replace:
            return re.sub(pattern, tag, " " + s + " ")
        return re.findall(pattern, " " + s + " ")

    def twitter_tokenizer(self, line):  # precessing of texts
...
# truncation of text if lengh exceeds limiting lenghth
...
class CustomBERTModel(nn.Module):  # pretrained bert model
    def __init__(self):
...
def evaluate(model, eval_dataloader):
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        predicted_labels = predicted_labels + logits.tolist()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()  # recommended command-line parsing module
    ...
    if args.do_train:
        ...
        test_data = TensorDataset(
            all_test_input_ids, all_test_input_mask, all_test_segment_ids, all_test_label_ids)
